...
export default function PatientEducationPage() {
  ...
  const currentRisk =
    currentStep > 0 && currentStep <= 5 ? config.risks[currentStep - 1] : null;
    ...
    <div className="min-h-screen bg-[#070707] text-white flex flex-col safe-area-inset">
      ...
      <header className="px-6 py-4 flex justify-between items-center border-b border-white/5 bg-black/50 backdrop-blur-xl sticky top-0 z-50">
        <div
          ...
          className="flex items-center gap-2 select-none cursor-pointer active:scale-95 transition-transform"
        >
        ...
        <div className="mx-6 mt-4 p-4 rounded-2xl bg-red-950/40 border border-red-500/30 animate-in zoom-in duration-500">
          <div className="flex items-center gap-3 text-red-400 mb-1">
            ...
            <h3 className="font-bold text-sm tracking-wide uppercase">
              Kin Consent Required
            </h3>
          </div>
          <p className="text-xs text-red-300/80 leading-relaxed">
            You are providing medical consent for{" "}
            <strong>{session.patient_name}</strong> as their{" "}
            <strong>{session.kin_relation}</strong> because the patient is
            currently unable to consent.
          </p>
        </div>
      )}
        ...
        {currentStep === 0 ? (
          <div className="space-y-6">
            <h1 className="text-3xl font-bold tracking-tight">
              Welcome,{" "}
              <span className="text-blue-500">{session.patient_name}</span>.
            </h1>
            <p className="text-gray-400 leading-relaxed text-lg">
              We will now walk you through the details of your{" "}
          ...
          <div className="space-y-8 flex flex-col items-center justify-center flex-1 text-center">
            ...
            <p className="text-gray-400 text-lg">
              You have completed the educational section. Next, we will verify
              your identity to finalize the consent.
            </p>
            <PillButton
              onClick={() => handleNext()}
              className="w-full h-16 text-lg font-bold"
